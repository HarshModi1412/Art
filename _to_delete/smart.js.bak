...
const MODULES = [
  ...
  { id: "supply",     name: "Supply Management",      sub: "Track inventory, get reorder points from your sales × lead times, and auto-generate purchase orders.", ico: "📦", cls: "tile-supply",   needs: null,     tag: "SUPPLY" },
...
function renderSupply(d) {
  ...
  const salesNote = meta.has_sales
    ? `Average daily sales are computed from your Sales data over ${meta.days_span} day${meta.days_span === 1 ? "" : "s"}. Reorder point = avg daily sales × lead time + safety stock.`
    : `No Sales data yet — upload Sales (from the home screen) to compute average daily usage automatically. You can still track stock and set lead time / safety stock manually.`;

  const rows = items.length ? items.map((it) => `
      <tr class="${it.below_reorder ? "sup-below" : ""}">
        <td>${esc(it.sku)}${it.supplier ? `<div class="muted tiny">${esc(it.supplier)}</div>` : ""}</td>
        <td class="num">${fmt(it.current_stock)}</td>
        ...
        <td class="num">${it.days_of_cover == null ? "—" : it.days_of_cover}</td>
        <td>${_supBadge(it)}</td>
        <td class="num">${it.below_reorder ? fmt(it.suggested_qty) : "—"}</td>
        <td class="sup-actions">
          <button class="btn ghost tiny" data-edit="${it.id}" title="Edit">✎</button>
          <button class="btn ghost tiny" data-del="${it.id}" title="Remove">🗑</button>
        </td>
      </tr>`).join("")
    : `<tr><td colspan="10" class="ap-empty">No inventory yet. Add an item, or pull products from your sales.</td></tr>`;
  ...
  const poRows = pos.length ? pos.slice().reverse().map((p) => `
      <tr>
        ...
        <td class="num">${p.total_value == null ? "—" : "₹" + fmt(p.total_value)}</td>
        <td><button class="btn ghost tiny" data-podl="${esc(p.po_number)}">⬇ Excel</button></td>
      </tr>`).join("")
    : `<tr><td colspan="6" class="ap-empty">No purchase orders yet. Generate one when items fall below their reorder point.</td></tr>`;
  ...
  const body = `
    ...
    <div class="row" style="display:flex;gap:8px;flex-wrap:wrap;margin:10px 0 6px;">
      ...
      ${belowN ? `<button class="btn approve sm" id="supGenPo">📦 Generate purchase order (${belowN})</button>` : ""}
    </div>
    ${belowN ? `<div class="action-card warning" style="margin-bottom:12px;"><div class="do">⚠ ${belowN} item${belowN === 1 ? "" : "s"} at or below reorder point</div><div class="why">Approve the “Reorder” card in the panel, or use “Generate purchase order” to create &amp; download a PO now.</div></div>` : ""}

    <div id="supForm" hidden></div>

    <div class="section-title" style="margin-top:8px;">Inventory</div>
    <div class="table-scroll">
      <table class="sup-table">
        <thead><tr>
          <th>Item</th><th class="num">Stock</th><th class="num">Avg/day</th>
          <th class="num">Lead (d)</th><th class="num">Safety</th><th class="num">Reorder pt</th>
          <th class="num">Cover (d)</th><th>Status</th><th class="num">Suggested qty</th><th></th>
        </tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </div>
    ...
    <div class="table-scroll">
      <table class="sup-table">
        <thead><tr><th>PO #</th><th>Created</th><th class="num">Items</th><th class="num">Qty</th><th class="num">Value</th><th></th></tr></thead>
        <tbody>${poRows}</tbody>
      </table>
    </div>`;
  ...
  document.querySelectorAll("[data-podl]").forEach((b) => b.onclick = () => download(`/api/supply/po/${encodeURIComponent(b.dataset.podl)}/download`, `${b.dataset.podl}.xlsx`));
}
...
function openSupplyForm(id) {
  ...
  f.innerHTML = `
    <div class="card sup-form">
      ...
      <div class="sup-form-grid">
        <label>Item name<input id="sfSku" value="${it ? esc(it.sku) : ""}" placeholder="e.g. Silver hoop earrings" /></label>
        <label>Supplier <span class="muted tiny">(optional)</span><input id="sfSup" value="${it ? esc(it.supplier || "") : ""}" /></label>
        <label>Current stock<input id="sfStock" type="number" min="0" step="any" value="${it ? it.current_stock : 0}" /></label>
        <label>Lead time (days)<input id="sfLead" type="number" min="0" step="any" value="${it ? it.lead_time_days : 7}" /></label>
        <label>Safety stock<input id="sfSafe" type="number" min="0" step="any" value="${it ? it.safety_stock : 0}" /></label>
        <label>Reorder qty <span class="muted tiny">(blank = auto)</span><input id="sfQty" type="number" min="0" step="any" value="${it && it.reorder_qty != null ? it.reorder_qty : ""}" /></label>
        <label>Unit cost ₹ <span class="muted tiny">(optional)</span><input id="sfCost" type="number" min="0" step="any" value="${it && it.unit_cost != null ? it.unit_cost : ""}" /></label>
      </div>
  ...
  $("sfSave").onclick = async () => {
    ...
    const payload = {
      ...
      reorder_qty: $("sfQty").value === "" ? null : parseFloat($("sfQty").value),
      unit_cost: $("sfCost").value === "" ? null : parseFloat($("sfCost").value),
    };
    if (!payload.sku) { const e = $("sfErr"); e.textContent = "Item name is required."; e.hidden = false; return; }
    try {
      const d = await api("/api/supply/item", { method: "POST", json: payload });
      _supAfter(d);
      toast("Saved");
...
async function supplyGeneratePo() {
  try {
    ...
    if (d.download_url) await download(d.download_url, `${d.po_number}.xlsx`);
    _supAfter(d);
    toast(`✅ ${d.po_number} generated — Excel downloaded & saved.`);
  } catch (e) { toast(e.message); }
}
