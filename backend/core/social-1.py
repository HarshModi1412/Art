...
FESTIVALS_2026 = [
    {"key": "ganesh_chaturthi", "date": "2026-09-14", "name": "Ganesh Chaturthi",
     "lead": 10, "span": 2, "categories": ["clothing"],
     "note": "Regionally concentrated — Maharashtra, Goa, Karnataka."},
    # span was 10 (the full festival to Anant Chaturdashi); for a clothing
    # seller the buying intent is the run-up and the day, so the campaign closes
    # ~2 days after — otherwise it lingered for over a week and kept outranking
    # the Navratri run-up that opens on the 20th.
...
def occasion_for(day: date, category: str = "") -> dict | None:
    ...
    Chauth and fourteen from Diwali, and both run-ups are live. An UPCOMING
    festival always wins over one whose day has already passed, and among
    upcoming ones the NEAREST wins — that is the festival the customer is
    shopping for now.

    THE BUG THIS FIXES: ranking purely by nearest date kept a just-passed
    festival (e.g. Ganesh Chaturthi, still inside its post-day span) ahead of
    the next one whose run-up had just opened (Navratri). Once a festival's day
    is behind us, the campaign that matters is the next one — so a passed
    festival is only chosen when nothing upcoming covers the day (i.e. we are
    genuinely still inside a multi-day festival like Navratri itself)."""
    ...
    for f in FESTIVALS_2026:
        ...
        if not (d - timedelta(days=lead) <= day <= d + timedelta(days=span)):
            continue
        # Sort key: (0 for today-or-upcoming, 1 for passed) then nearest. So an
        # upcoming run-up outranks a passed festival's tail regardless of gap.
        passed = 1 if d < day else 0
        key = (passed, abs((d - day).days))
