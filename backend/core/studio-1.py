...
def _openai_image(prompt: str, reference: tuple[bytes, str] | None,
    ...
    # Cost control. On gpt-image-1 the `quality` tier is the single biggest lever
    # on price: "high" (the old implicit default) is ~₹14/image, "medium" ~₹3.5,
    # "low" ~₹1. Three "high" images ran close to a dollar, which is what the
    # seller hit. Default to "medium" — a clear step down in cost with little
    # visible loss on a 1024² product shot — and let a deployment override it, or
    # switch the whole model to the cheaper "gpt-image-1-mini", via env.
    #   OPENAI_IMAGE_QUALITY = low | medium | high | auto   (default: medium)
    #   OPENAI_IMAGE_MODEL   = gpt-image-1 | gpt-image-1-mini | ...
    quality = (os.environ.get("OPENAI_IMAGE_QUALITY") or "medium").strip().lower()
    if reference:
        ...
        r = client.images.edit(
            ...
            prompt=prompt, size="1024x1024", input_fidelity="high", quality=quality, n=1)
        from_ref = True
    else:
