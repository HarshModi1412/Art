...
def opportunities(week_start: date, category: str = "") -> list[dict]:
    ...
    for f in social.FESTIVALS_2026:
        ...
        if start <= week_end and end >= week_start:
            ...
            # A festival whose day is already behind the week start is in its
            # tail: it must rank BELOW an upcoming festival whose run-up is live,
            # so the brief leads with the campaign that's actually ahead
            # (Navratri) rather than the one that just passed (Ganesh).
            passed = d < week_start
            out.append({
                "kind": "festival", "priority": 2 if passed else 1,
                "_gap": abs((d - week_start).days), "key": f.get("key", ""),
    ...
    return sorted(out, key=lambda o: (o["priority"], o.get("_gap", 999)))
